...
- 返款：一笔快递可以分多次登记实际返款。
- 返利：每个报单商品项默认按预计全额计入，可单独调整实际返利（含 0），也可恢复默认；结单后仍可调整。
...
快递可以“结单”标记返款已完成；结单前需要登记一笔实际返款，金额可以是 `0.00`，结单时会显示实际返款与预计返款的差额。结单后不能继续编辑快递或返款，撤销结单后恢复编辑。已出库或已退款商品的名称、数量、付款和预计金额也不能回改，以免破坏历史统计；备注仍可修改。实际返利通过报单的「调整返利」独立修改，填写原整批总额，退款份额自动剔除；默认全额不等于已经到账。详见 [`docs/ACTUAL_REBATE.md`](docs/ACTUAL_REBATE.md)。
...
0.1.4 增加独立实际返利调整、默认全额与恢复默认，并修复特殊商品名快递编辑丢行、前序快递修改影响已结单金额分摊、极大金额格式化误差和网页旧脚本缓存问题。后端和 APK 应一起更新，旧 APK 仍显示旧的预计返利利润口径。
